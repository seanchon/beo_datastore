...
@attr.s(frozen=True)
class AggregateBatterySimulation(DERSimulation):
    """
    ...
    def __add__(self, other):
        ...
        combined_results.update(other.results)
    ...
    def _generate_battery_simulation(
        ...
        """
        battery_simuation = FixedScheduleBatterySimulation(
            battery=battery,
            load_intervalframe=load_intervalframe,
            charge_schedule=charge_schedule,
            discharge_schedule=discharge_schedule,
        )
        battery_simuation.generate_full_sequence()

        return battery_simuation
...
class AggregateBillCalculator(object):
    ...
    simulation = attr.ib(validator=instance_of(DERSimulation))
    date_ranges = attr.ib()
    rate_plan = attr.ib()
    ...
    def __attrs_post_init__(self):
        object.__setattr__(
            self,
            "before_bills",
            self.generate_bills(self.simulation.pre_intervalframes),
        )
        object.__setattr__(
            self,
            "after_bills",
            self.generate_bills(self.simulation.post_intervalframes),
        )
    ...
    def pre_DER_bill_totals(self):
        ...
        df.columns = self.simulation.meters
...
class AggregateGHGCalculator(object):
    ...
    def ghg_before_frame288(self):
        return (
            self.simulation.aggregate_pre_intervalframe.total_frame288
            * self.ghg_frame288
        )
    ...
    def ghg_after_frame288(self):
        return (
            self.simulation.aggregate_post_intervalframe.total_frame288
            * self.ghg_frame288
        )
